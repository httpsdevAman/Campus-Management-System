...
export default function AuthorityGrievanceDetail() {
  ...
  async function addRemarkOnly() {
    ...
    setSubmitting(true);
    const updated = await addGrievanceComment({ id, message: remark, by: user });
    setG(updated);
    setRemark("");
    setSubmitting(false);
  }
  ...
  if (!g) {
    return (
      <div className="text-center py-16">
        ...
        <button onClick={() => navigate("/authority/grievances")} className="mt-3 text-sm text-indigo-500 hover:underline cursor-pointer">
          Back to grievances
        </button>
      </div>
    );
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, ease: "easeOut" }}>
      ...
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        ...
        <button onClick={() => navigate("/authority/grievances")} className="rounded-xl border border-slate-200/60 dark:border-white/[0.06] bg-white/40 dark:bg-white/[0.03] px-4 py-2 text-sm font-semibold text-slate-600 dark:text-white/60 hover:bg-slate-50 dark:hover:bg-white/[0.05] transition-colors cursor-pointer">
          Back
        </button>
      </div>
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        ...
        <div className="space-y-5">
          ...
          {/* Actions card */}
          <div className="rounded-2xl border border-slate-200/60 dark:border-white/[0.06] bg-white/60 dark:bg-white/[0.02] p-5 transition-colors duration-300">
            <h3 className="mb-3 text-sm font-semibold text-slate-800 dark:text-white/90">Actions</h3>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className="rounded-xl bg-indigo-500 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-600 disabled:opacity-60 transition-colors cursor-pointer"
              >
              ...
              <button
                ...
                className="rounded-xl border border-slate-200/60 dark:border-white/[0.06] bg-white/40 dark:bg-white/[0.03] px-4 py-2 text-sm font-semibold text-slate-600 dark:text-white/60 hover:bg-slate-50 dark:hover:bg-white/[0.05] disabled:opacity-60 transition-colors cursor-pointer"
              >
                Add Remark Only
              </button>
            </div>
