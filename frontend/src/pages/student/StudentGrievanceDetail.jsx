...
export default function StudentGrievanceDetail() {
  const { id } = useParams();
  ...
  if (!g) {
    return (
      <div className="text-center py-16">
        ...
        <button onClick={() => navigate("/student/grievances")} className="mt-3 text-sm text-indigo-500 hover:underline cursor-pointer">
          Back to grievances
        </button>
      </div>
    );
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, ease: "easeOut" }}>
      ...
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        ...
        <button onClick={() => navigate("/student/grievances")} className="rounded-xl border border-slate-200/60 dark:border-white/[0.06] bg-white/40 dark:bg-white/[0.03] px-4 py-2 text-sm font-semibold text-slate-600 dark:text-white/60 hover:bg-slate-50 dark:hover:bg-white/[0.05] transition-colors cursor-pointer">
          Back
        </button>
      </div>
