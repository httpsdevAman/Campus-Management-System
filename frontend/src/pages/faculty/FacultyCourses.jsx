...
export default function FacultyCourses() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      const data = await listCoursesForFaculty(user.id);
    ...
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, ease: "easeOut" }}>
      <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-white/40">Create and manage courses you teach.</p>
        </div>
        ...
        <motion.button
          ...
          className="rounded-xl bg-indigo-500 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-indigo-600 transition"
        >
          + New Course
        </motion.button>
      </div>
        ...
        <motion.div key={`${dept}-${q}`} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.15 }}>
          {loading ? (
            ...
            <div className="rounded-2xl border border-slate-200/60 dark:border-white/[0.06] bg-white/60 dark:bg-white/[0.02] p-10 text-center">
              <p className="text-lg font-semibold text-slate-800 dark:text-white/85">No courses yet</p>
              <p className="mt-1.5 text-sm text-slate-500 dark:text-white/40">Create your first course to add materials and assignments.</p>
              <button
                onClick={() => navigate("/faculty/courses/new")}
            ...
            <>
              <CourseTable rows={filtered} onOpen={(cid) => navigate(`/faculty/courses/${cid}`)} />
              <div className="md:hidden grid gap-3">
                {filtered.map((c, i) => (
                  <CourseCard key={c.id} course={c} index={i} onClick={() => navigate(`/faculty/courses/${c.id}`)} />
                ))}
              </div>
            </>
          )}
